...
const DeliveryOrderList = () => {
  ...
  /**
   * 单个订单修改状态的mutation（使用封装的接口）
   * 成功后自动刷新订单列表
   */
  const mutation = useMutation({
    mutationFn: updateOrderStatusToCompleted,
    onSuccess: async () => {
      // 单个订单状态更新成功，刷新数据
      console.log("单个订单状态更新成功，开始刷新数据");
      // 1. 先执行原有的refetch刷新列表
      await refetch();
      // 2. 额外手动调用一次列表接口（确保最新数据）
      const extraResponse = await getInprocessOrders({
        page,
        pageSize,
        search: searchTerm
      });
      console.log("额外调用列表接口结果：", extraResponse);
      // 3. 清空选中状态
      setSelectedOrderIds([]);
  ...
  /**
   * 查看发货单详情
   * @param {string|number} deliveryOrderId 发货单ID
   */
          ...
          {selectedDeliveryOrder && selectedDeliveryOrder.orders.length > 0 ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="mt-2 text-gray-600">
                请点击上方 <Eye className="inline h-4 w-4 text-blue-600" /> 按钮查看发货单详情
              </p>
            </div>
          )}
        </CardContent>
