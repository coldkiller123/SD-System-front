...
import { useEffect } from 'react';
// 导入两个接口
import { createOrder, updateOrder } from '@/apis/main';
...
const SalesOrderForm = ({ initialData, onSuccess, onCancel }) => {
  ...
  useEffect(() => {
    calculateTotal();
  }, [form.getValues('quantity'), form.getValues('unitPrice'), form.getValues('status')]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (initialData) {
        ...
        const updateData = {
          ...data,
          modifiedAt: new Date().toISOString(), // 生成ISO格式时间戳
          modifiedBy: '当前用户', // 实际项目中应替换为登录用户
          // 编辑时不修改创建时间
          createdAt: initialData.createdAt
        };
        // 调用更新接口（PUT /api/orders/{id}）
        result = await updateOrder(data.id, updateData);
      } else {
        // 新建订单：生成创建时间
        const createData = {
          ...data,
